...
- [ ] **Directory Structure:** Inside the `backend/core/` application, create the following directory structure if it doesn't exist: `management/commands/`. Ensure both directories contain an `__init__.py` file.
- [ ] **Command Creation:** Create a file named `seed_zoo.py` inside `management/commands/`.
- [ ] **Implementation:** Implement the `BaseCommand` to create the Tenant and CustomFields. Use the exact script provided below.
- [ ] **Execution:** Run the command in the terminal via `python manage.py seed_zoo`.
